...
def process_image(args_tuple):
    ...
    (filename, input_path, output_path, method, leaf_size, threshold,
     label, no_overlay, fill_alpha, show_borders, include_legend) = args_tuple
...
def main():
    ...
    print(f"Method: {args.method} | Leaf size: {leaf_size_display}px | "
          f"Threshold: {args.threshold or 'off'} | Workers: {workers}\n")
    ...
    if not tasks:
        print("No images found across all input folders.")
        sys.exit(1)
